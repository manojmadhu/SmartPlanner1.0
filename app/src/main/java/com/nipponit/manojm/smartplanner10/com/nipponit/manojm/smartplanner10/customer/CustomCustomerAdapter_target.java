...
public class CustomCustomerAdapter_target extends ArrayAdapter<CustomerClass_target> {

    private Context context;
    private ArrayList<CustomerClass_target> dataset;

    private ArrayList<CustomerClass_target> Original_List;
    private ArrayList<CustomerClass_target> Customer_List;
    ...
    public CustomCustomerAdapter_target(Context context, ArrayList<CustomerClass_target> data) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        CustomerClass_target customerModel = getItem(position);
    ...
    private class CustomerFilter extends Filter{
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if(constraint != null && constraint.toString().length() > 0 ){
                ArrayList<CustomerClass_target> filtered_list = new ArrayList<>();
                for(int i = 0, l = Original_List.size(); i < l; i++){
                    if((Original_List.get(i).getCname_().toUpperCase()).contains(constraint.toString().toUpperCase())){
                        CustomerClass_target customerClass_target = new CustomerClass_target(Original_List.get(i).getCcode_(),
                                Original_List.get(i).getCname_(),Original_List.get(i).getCcity_(),Original_List.get(i).getTarget_());

                        filtered_list.add(customerClass_target);
                    }
                }
        ...
        protected void publishResults(CharSequence constraint, FilterResults results) {
            Customer_List = (ArrayList<CustomerClass_target>)results.values;
